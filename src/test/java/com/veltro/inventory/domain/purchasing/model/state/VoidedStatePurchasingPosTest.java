...
/**
 * Unit tests for {@link VoidedStatePurchasing} (B2-04 - State Pattern).
 *
 * <p>Tests that VOIDED state is terminal - no operations are allowed.
 */
class VoidedStatePurchasingPosTest {

    private VoidedStatePurchasing state;
    private PurchaseOrderEntity order;

    @BeforeEach
    void setUp() {
        state = new VoidedStatePurchasing();
