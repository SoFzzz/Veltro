...
/**
 * Unit tests for {@link PurchaseOrderVoidedState} (B2-04 - State Pattern).
 *
 * <p>Tests that VOIDED state is terminal - no operations are allowed.
 */
class PurchaseOrderVoidedStateTest {

    private PurchaseOrderVoidedState state;
    private PurchaseOrderEntity order;

    @BeforeEach
    void setUp() {
        state = new PurchaseOrderVoidedState();
    ...
    void voidOrder_voidedState_throwsInvalidStateTransition() {
        assertThatThrownBy(() -> state.voidOrder(order))
                .isInstanceOf(InvalidStateTransitionException.class)
                .hasMessageContaining("Cannot void purchase order in VOIDED status");
    }
