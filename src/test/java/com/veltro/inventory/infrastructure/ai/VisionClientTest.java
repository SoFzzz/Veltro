...
/**
 * Unit tests for {@link VisionClient} (B3-01).
 */
@ExtendWith(MockitoExtension.class)
@DisplayName("VisionClient")
class VisionClientTest {
    ...
    void analyzeProductImage_notConfigured_returnsEmptySuggestions() {
        when(visionApiConfig.isConfigured()).thenReturn(false);
    ...
    void analyzeProductImage_emptyImage_returnsEmptySuggestions() {
        when(visionApiConfig.isConfigured()).thenReturn(true);
    ...
    void analyzeProductImage_imageTooLarge_returnsEmptySuggestions() {
        when(visionApiConfig.isConfigured()).thenReturn(true);
        when(visionApiConfig.getMaxImageSizeMb()).thenReturn(10);
    ...
    void analyzeProductImage_enrichesSuggestionWhenMatchExists() {
        when(visionApiConfig.isConfigured()).thenReturn(true);
        when(visionApiConfig.getApiKey()).thenReturn("AIza-test-key");
        when(visionApiConfig.getModel()).thenReturn("gemini-2.5-flash");
        when(visionApiConfig.getMaxTokens()).thenReturn(200);
        when(visionApiConfig.getMaxRetries()).thenReturn(1);
