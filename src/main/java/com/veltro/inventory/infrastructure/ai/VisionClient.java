 ...
 * </ul>
 *
 * @see VisionApiConfig for configuration
 */
@Slf4j
@Component
...
public class VisionClient {
    ...
    private final VisionApiConfig config;
    ...
    public ProductSuggestionResponse analyzeProductImage(MultipartFile image) {
        ...
        try {
            ...
            VisionApiResponse response = callVisionApi(imageBase64, imageMediaType);
            long processingTime = System.currentTimeMillis() - startTime;

            if (response == null) {
                log.warn("Vision API returned null response");
                return ProductSuggestionResponse.empty("AI_VISION", processingTime);
            }

            ProductSuggestionResponse suggestions = parseVisionApiResponse(response, processingTime);
    ...
    private VisionApiResponse callVisionApi(String imageBase64, String imageMediaType) {
     ...
     */
    private VisionApiResponse callGeminiApi(String imageBase64, String imageMediaType) {
        int attempt = 0;
        ...
        while (attempt < config.getMaxRetries()) {
            try {
                ...
                // Parse Gemini response and convert to VisionApiResponse format
                VisionApiResponse response = parseGeminiResponse(responseJson);
    ...
    /**
     * Parses Gemini API response and converts to VisionApiResponse format.
     */
    private VisionApiResponse parseGeminiResponse(String responseJson) throws Exception {
        ...
        // Convert to VisionApiResponse format for compatibility
        VisionApiResponse response = new VisionApiResponse();
        response.choices = new ArrayList<>();
        VisionApiResponse.Choice choice = new VisionApiResponse.Choice();
        choice.message = new VisionApiResponse.Choice.Message();
     ...
     */
    private VisionApiResponse callOpenAiApiInternal(String imageBase64, String imageMediaType) {
        int attempt = 0;
        Exception lastException = null;
        ...
        while (attempt < config.getMaxRetries()) {
            try {
                ...
                VisionApiRequest request = buildVisionApiRequest(imageBase64, imageMediaType);
                HttpHeaders headers = new HttpHeaders();
                headers.setContentType(MediaType.APPLICATION_JSON);
                headers.set("Authorization", "Bearer " + config.getApiKey());

                HttpEntity<VisionApiRequest> entity = new HttpEntity<>(request, headers);
                VisionApiResponse response = restTemplate.postForObject(apiEndpoint, entity, VisionApiResponse.class);
                ...
                lastException = e;
                log.warn("Vision API failed on attempt {} of {}: {}", attempt, config.getMaxRetries(), e.getMessage());
        ...
        log.error("Vision API call failed after {} attempts", config.getMaxRetries(), lastException);
        return null;
    }

    private VisionApiRequest buildVisionApiRequest(String imageBase64, String imageMediaType) {
        ...
        List<Object> userContent = List.of(
                new VisionApiRequest.TextContent(USER_PROMPT),
                new VisionApiRequest.ImageContent(new VisionApiRequest.ImageUrl(imageUrl, "low"))
        );

        List<Object> systemContent = List.of(
                new VisionApiRequest.TextContent(SYSTEM_PROMPT)
        );

        return new VisionApiRequest(
                config.getModel(),
                List.of(
                        new VisionApiRequest.Message("system", systemContent),
                        new VisionApiRequest.Message("user", userContent)
                ),
                config.getMaxTokens(),
                0.3
        );
    ...
    private ProductSuggestionResponse parseVisionApiResponse(VisionApiResponse response, long processingTime) {
        if (response.choices == null || response.choices.isEmpty()) {
            log.warn("Vision API response has no choices");
            return ProductSuggestionResponse.empty("AI_VISION", processingTime);
        }

        String contentText = response.choices.get(0).message.content;
        log.debug("Raw Vision API response: {}", contentText);
    ...
    // Vision API request format uses a more complex structure
    // Text: {"type": "text", "text": "..."}
    // Image: {"type": "image_url", "image_url": {"url": "...", "detail": "..."}}
    
    record VisionApiRequest(
            String model,
    ...
    static class VisionApiResponse {
        List<Choice> choices;
